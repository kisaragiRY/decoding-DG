...
def test_SmoothedSpikesDataset(data_dir, coord_axis, window_size):
    ...
    train_ratio = .8
    (X_train, y_train), (X_test, y_test) = dataset.load_all_data(window_size, train_ratio)
    ...
    (ori_X_train, ori_y_train), (ori_X_test, ori_y_test) = dataset.split_data(ori_spikes, ori_coord, train_ratio)

    assert (X_train[:,1:] == filter_spikes(window_size, ori_X_train)).all()
    assert (y_train == ori_y_train).all()
...
#     design_matrix, coord = SummedSpikesDataset(data_dir, False, False).load_all_data(coord_axis, nthist, window_size)
...
if __name__ == "__main__":
    DATA_ROOT = Path('data/alldata/')
    coord_axis, window_size = "y-axis", 3
    test_SmoothedSpikesDataset(next(iter(DATA_ROOT.iterdir())), coord_axis, window_size)