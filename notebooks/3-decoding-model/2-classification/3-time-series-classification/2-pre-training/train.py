...
from rocket import Rocket as Rocket_2d
...
def rocket_2d_trainer_tuning(data_dir, K_range, kernels_range, note):
    """The training script.
    """
    data_name = str(data_dir).split('/')[-1]
    res_all = []
    for K, num_kernels in tqdm(product(K_range, kernels_range)):
        ...
        # transform
        transform_pipeline = Pipeline([
            ("rocket", Rocket_2d(num_kernels, ParamData().kernel_dim, random_state=ParamData().random_state)),
            ("std_scaler", StandardScaler()),
            ("l2_norm", Normalizer()),
        ])
        X_train = transform_pipeline.fit_transform(X_train)
    ...
    if not (ParamDir().output_dir/data_name).exists():
        (ParamDir().output_dir/data_name).mkdir()
    with open(ParamDir().output_dir/data_name/(f"tsc_2d_rocket_tuning_{ParamTrain().model_name}_{note}.pickle"),"wb") as f:
        pickle.dump(res_all, f)
...
if __name__ == "__main__":
    ...
    # Parallel(n_jobs=-1)(delayed(
    #     rocket_2d_trainer_tuning(data_dir, K_range, kernels_range, "large_scale")
    #     )(data_dir) for data_dir in ParamDir().data_path_list)

    # ---- small scale tuning ----
    K_range = [16]
    kernels_range = range(20, 300, 5)
    # # rocket_trainer_tuning(K_range, kernels_range, "small_scale")
    # Parallel(n_jobs=-1)(delayed(
    #     rocket_trainer_tuning(data_dir, K_range, kernels_range, "small_scale")
    #     )(data_dir) for data_dir in ParamDir().data_path_list)
    Parallel(n_jobs=-1)(delayed(
    rocket_2d_trainer_tuning(data_dir, K_range, kernels_range, "small_scale")
    )(data_dir) for data_dir in ParamDir().data_path_list)