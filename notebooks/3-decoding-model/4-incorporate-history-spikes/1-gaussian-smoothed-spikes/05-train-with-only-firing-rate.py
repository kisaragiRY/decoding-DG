...
def main() -> None:
    ...
    for data_dir in tqdm(ParamDir().data_path_list):
        ...
        results_all=[]
        for coord_axis in ParamFiringRateData().coord_axis_opts:
            dataset = SmoothedSpikesDataset(data_dir, coord_axis, False, False)
            for window_size in ParamFiringRateData().window_size_range:
        ...
        with open(ParamDir().output_dir/data_name/(f"rr_training_firing_rate.pickle"),"wb") as f:
